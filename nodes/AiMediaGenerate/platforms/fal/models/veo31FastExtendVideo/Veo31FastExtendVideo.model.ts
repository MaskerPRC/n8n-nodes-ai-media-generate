...
export class Veo31FastExtendVideoModel extends BaseFalModel {
	getConfig(): ModelConfig {
		return {
			name: 'veo31FastExtendVideo',
		...
		return [
			{
				...
				required: true,
				description:
					'URL of the video to extend. The video should be 720p or 1080p resolution in 16:9 or 9:16 aspect ratio. Supports publicly accessible URLs or base64 data URIs.',
			},
			{
				displayName: 'Prompt',
				name: 'prompt',
				...
				required: true,
				description:
					'The text prompt describing how the video should be extended. Include: Action (what should happen next), Style (maintain or transition visual style), Camera motion (optional), and Ambiance (optional).',
			},
			{
	...
	async buildRequestParams(): Promise<IDataObject> {
		...
		// 可选参数：generate_audio
		const generateAudio = this.executeFunctions.getNodeParameter('generate_audio', this.itemIndex) as boolean;
	...
	protected processSyncResponse(response: IDataObject): IDataObject {
		// Veo 3.1 Fast Extend Video 不支持同步请求
		return response;
	}
