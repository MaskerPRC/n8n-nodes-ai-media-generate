...
export class Flux1DevI2IModel extends BaseFalModel {
	getConfig(): ModelConfig {
		return {
			name: 'flux1DevI2I',
			displayName: 'FLUX.1 [dev] Image-to-Image',
	...
	getInputSchema(): INodeProperties[] {
		return [
			...
			},
			{
				...
				required: true,
				description:
					'The URL of the image to generate an image from. Can be publicly accessible URL or base64 data URI.',
			},
			{
	...
	async buildRequestParams(): Promise<IDataObject> {
		...
		// 可选参数
		const strength = this.executeFunctions.getNodeParameter('strength', this.itemIndex) as number;
