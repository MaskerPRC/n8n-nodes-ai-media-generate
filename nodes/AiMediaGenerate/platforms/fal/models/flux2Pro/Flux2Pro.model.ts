...
export class Flux2ProModel extends BaseFalModel {
	...
	getInputSchema(): INodeProperties[] {
		return [
			...
			},
			{
				...
				default: '2',
				description: 'The safety tolerance level for the generated image. 1 being the most strict and 5 being the most permissive. Note: This property is only available through API calls.',
			},
			{
	...
	async buildRequestParams(): Promise<IDataObject> {
		...
		const safetyTolerance = this.executeFunctions.getNodeParameter('safety_tolerance', this.itemIndex) as string;
		if (safetyTolerance) {
