...
export class Moondream3PreviewDetectModel extends BaseFalModel {
	getConfig(): ModelConfig {
		return {
			name: 'moondream3PreviewDetect',
			displayName: 'Moondream 3 Preview Detect',
	...
	protected processSyncResponse(response: IDataObject): IDataObject {
		// Moondream 3 Preview Detect 不支持同步请求
		return response;
	}

	protected processAsyncResponse(response: IDataObject): IDataObject {
		// 直接返回原始响应，保留所有字段（包括 objects, image, finish_reason, usage_info）
		return response;
	}
}
