...
export class VeedVideoBackgroundRemovalModel extends BaseFalModel {
	getConfig(): ModelConfig {
		return {
			name: 'veedVideoBackgroundRemoval',
			displayName: 'Veed Video Background Removal',
		...
		return [
			{
				...
				required: true,
				description:
					'URL of the video file to remove background from. Must be publicly accessible or base64 data URI.',
			},
			{
	...
	protected processSyncResponse(response: IDataObject): IDataObject {
		// Veed Video Background Removal 不支持同步请求
		return response;
	}
