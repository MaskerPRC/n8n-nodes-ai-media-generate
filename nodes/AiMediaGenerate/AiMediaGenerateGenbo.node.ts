...
import { getAllGenboModelProperties } from './platforms/genbo/modelRegistry';

export class AiMediaGenerateGenbo implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'AI Media Generate (Genbo)',
		name: 'aiMediaGenerateGenbo',
		icon: { light: 'file:genbo.svg', dark: 'file:genbo.dark.svg' },
		group: ['transform'],
		version: 1,
		subtitle: '={{$parameter["model"]}}',
		description: 'Generate AI media content (images, videos, and audio) using Genbo platform',
		defaults: {
			name: 'AI Media Generate (Genbo)',
		},
		usableAsTool: true,
	...
	async getModelOptions(this: ILoadOptionsFunctions): Promise<Array<{ name: string; value: string }>> {
		...
		const platformConfig = getPlatformConfig('genbo');
		if (!platformConfig) {
			return [];
		}
	...
	private getDynamicProperties(): INodeProperties[] {
		...
		return getAllGenboModelProperties();
	}
			...
			try {
				...
				if (!platformConfig) {
					throw new NodeOperationError(
						this.getNode(),
						'Genbo platform is not supported',
						{ itemIndex },
					);
				}
